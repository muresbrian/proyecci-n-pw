...
OUTPUT_FILE = 'Reporte_Salud_Avanzado_2026.xlsx'
INACTIVITY_WEEKS = 4
VARIATION_THRESHOLD = 15.0

def generar_reporte_avanzado():
    print("Iniciando análisis de Health Score Avanzado 2026...")
    ...
    # Cargar y cruzar con Vendedor desde Ranking.csv
    print("Cargando Vendedores desde Ranking.csv...")
    vendedor_map = {}
    ...
    # Calcular el volumen total de 2026
    ...
    for idx, row in df_26.iterrows():
        ...
        # Analizar comportamiento de las semanas desde que empezó en el rango de últimas 12 semanas (Sem 14 a 25)
    ...
    ws_dash['A1'] = "DASHBOARD DE SALUD COMERCIAL 2026"
    ws_dash['A1'].font = title_font
    ws_dash['A2'] = f"Seguimiento preventivo y plan de acción de la cartera total activa de 2026 ({n_clients} clientes únicos)"
    ws_dash['A2'].font = desc_font
    
    # Tarjetas KPI superiores (A4:D6)
    # Card 1: ALERTA ROJA (Ha bajado o Perdido Clase A)
    # Card 2: ALERTA NARANJA (Ha bajado o Perdido Clase B)
    # Card 3: ESTABLES (Constante)
    # Card 4: CRECIENDO (Ha subido)
    kpis_config = [
        {"col": 1, "label": "ALERTAS ROJAS 🚨", "formula": "=SUM(I14:J14)", "fill": fill_red, "font": font_red_kpi},
        {"col": 2, "label": "ALERTAS NARANJAS ⚠️", "formula": "=SUM(I15:J15)", "fill": fill_red, "font": font_red_kpi},  # Coloreado suave naranja/rojo
    ...
    for k in kpis_config:
        ...
        # Mostrar el porcentaje en la fila 6 relativo al total (celda G9)
    ...
    # Escribir Tabla de Resumen (F4:H9)
    ...
    ws_dash.cell(row=9, column=6, value="Total Cartera").font = bold_font
    ...
    # Escribir Matriz de Riesgo Comercial (F11:K17)
    ws_dash.cell(row=11, column=6, value="MATRIZ DE RIESGO COMERCIAL (Segmentación ABC vs Salud)").font = Font(name=font_family, size=11, bold=True, color="1F4E78")
    ...
    for idx, segment in enumerate(matrix_rows):
        ...
        # COUNTIFS Formulas
        # Fila en Cartera Completa: Segmento en Col D (4), Estado de Salud en Col J (10)
        ...
        # Colorear según el riesgo (Ha bajado / Perdido de Clase A/B con rojo/naranja)
    ...
    # Fila de Totales de la Matriz (Fila 17)
    ...
    chart = DoughnutChart()
    chart.style = 10
    chart.title = "Salud General de la Cartera 2026"
    chart.width = 12.5
    chart.height = 9.5
    ...
    alert_sheets = [
        {
            ...
            'df': df_alertas_rojas,
            'desc': 'Clientes Clase A (Alto Valor) con caídas de volumen >15% o perdidos en 2026. Requieren acción comercial inmediata de Directores.'
        },
        {
            'name': 'Alertas Naranjas (Vendedores)',
            'df': df_alertas_naranjas,
            'desc': 'Clientes Clase B (Valor Medio) con caídas de volumen >15% o perdidos en 2026. Requieren seguimiento prioritario de Vendedores.'
        }
    ]
    ...
    for ash in alert_sheets:
        ...
        # Column widths
        ws.column_dimensions['A'].width = 32 # Holder
        ws.column_dimensions['B'].width = 22 # Director
        ws.column_dimensions['C'].width = 22 # Vendedor
        ws.column_dimensions['D'].width = 12 # Segmento
        ws.column_dimensions['E'].width = 18 # Promedio Inicial
        ws.column_dimensions['F'].width = 18 # Promedio Reciente
        ws.column_dimensions['G'].width = 12 # Variación
        ws.column_dimensions['H'].width = 22 # Pérdida Absoluta ($)
        ws.column_dimensions['I'].width = 14 # Health Score
        ws.column_dimensions['J'].width = 75 # Acción Comercial Sugerida
        
    # ----------------- HOJA 4: CARTERA COMPLETA -----------------
    ws_full = wb.create_sheet(title="Cartera Completa")
    ws_full.views.sheetView[0].showGridLines = True
    
    ws_full['A1'] = "CARTERA COMPLETA 2026 - HEALTH SCORE"
    ws_full['A1'].font = title_font
    ws_full['A2'] = f"Listado detallado de los {n_clients} comercios activos en 2026 con sus métricas completas"
    ...
    ws_full.column_dimensions['A'].width = 32 # Holder
    ws_full.column_dimensions['B'].width = 22 # Director
    ws_full.column_dimensions['C'].width = 22 # Vendedor
    ws_full.column_dimensions['D'].width = 15 # Segmento ABC
    ws_full.column_dimensions['E'].width = 25 # Promedio Inicial
    ws_full.column_dimensions['F'].width = 25 # Promedio Reciente
    ws_full.column_dimensions['G'].width = 15 # Variación
    ws_full.column_dimensions['H'].width = 22 # Pérdida Absoluta ($)
    ws_full.column_dimensions['I'].width = 22 # Health Score (0-100)
    ws_full.column_dimensions['J'].width = 16 # Estado
    ws_full.column_dimensions['K'].width = 35 # Prioridad
    ws_full.column_dimensions['L'].width = 75 # Acción
    ws_full.column_dimensions['M'].width = 22 # Última semana
    ws_full.column_dimensions['N'].width = 20 # Último mes
    
    # Añadir Semáforo visual en el Health Score y Variación
    # Rojo: F8CBAD, Amarillo: FFF2CC, Verde: C6E0B4
    ...
    try:
        wb.save(OUTPUT_FILE)
        print(f"Reporte de Health Score Avanzado generado con éxito en '{OUTPUT_FILE}'")
    except PermissionError:
        import time
        fallback_name = f"Reporte_Salud_Avanzado_2026_{int(time.time())}.xlsx"
        try:
